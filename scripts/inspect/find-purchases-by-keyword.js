...
loadRuntimeEnv({ entry: 'find-purchases-by-keyword' });

const keyword = String(process.argv[2] || '').trim();
if (!keyword) {
  console.error('Usage: node scripts/inspect/find-purchases-by-keyword.js <keyword>');
  process.exit(1);
}
...
(async () => {
  const rows = await scan('ft_purchases');
  const matched = rows.filter((row) => {
    const text = `${row.studentName || ''} ${row.notes || ''} ${row.id || ''}`;
