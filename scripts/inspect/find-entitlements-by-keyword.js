...
loadRuntimeEnv({ entry: 'find-entitlements-by-keyword' });

const keyword = String(process.argv[2] || '').trim();
if (!keyword) {
  console.error('Usage: node scripts/inspect/find-entitlements-by-keyword.js <keyword>');
  process.exit(1);
}
...
(async () => {
  const rows = await scan('ft_entitlements');
  const matched = rows.filter((row) => {
    const text = `${row.studentName || ''} ${row.packageName || ''} ${row.id || ''}`;
