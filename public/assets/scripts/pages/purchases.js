...
function getFilteredPurchases(){
  ...
  return purchases.filter(p=>{
    if(!searchHit(q,p.studentName,p.packageName,p.amountPaid,p.payMethod,p.purchaseDate,p.courseType,p.packageTimeBand,p.ownerCoach,p.productName))return false;
...
function purchasePackageSnapshotHtml(p){
  ...
  return `<div class="sec-ttl">购买时规则快照</div><div class="fgrid"><div class="fg"><div class="flabel">课包课时</div><div class="finput">${parseInt(p.packageLessons)||0} 节</div></div><div class="fg"><div class="flabel">课包标价</div><div class="finput">¥${fmt(p.packagePrice)}</div></div><div class="fg"><div class="flabel">课程类型</div><div class="finput">${esc(p.courseType)||'—'}</div></div><div class="fg"><div class="flabel">历史课程产品（辅助）</div><div class="finput">${esc(p.productName)||'—'}</div></div><div class="fg"><div class="flabel">归属教练</div><div class="finput">购买时选择</div></div><div class="fg"><div class="flabel">可上课教练</div><div class="finput">${esc(coachText)}</div></div><div class="fg"><div class="flabel">时段类型</div><div class="finput">${esc(p.packageTimeBand)||'全天'}</div></div><div class="fg"><div class="flabel">每日时段</div><div class="finput">${esc(windows)}</div></div><div class="fg"><div class="flabel">可用校区</div><div class="finput">${esc(campusText)}</div></div><div class="fg"><div class="flabel">使用开始</div><div class="finput">${esc(p.usageStartDate)||'—'}</div></div><div class="fg"><div class="flabel">使用结束</div><div class="finput">${esc(p.usageEndDate)||'—'}</div></div></div>`;
}
...
function openPurchaseDetailModal(id){
  ...
  document.getElementById('mBody').innerHTML=`<div class="sec-ttl">成交快照</div><div class="fgrid"><div class="fg"><div class="flabel">支付日期</div><div class="finput">${esc(p.purchaseDate)||'—'}</div></div><div class="fg"><div class="flabel">系统录入时间</div><div class="finput">${fmtDt(p.createdAt)}</div></div><div class="fg"><div class="flabel">学员</div><div class="finput">${esc(p.studentName)||'—'}</div></div><div class="fg"><div class="flabel">售卖课包</div><div class="finput">${esc(p.packageName)||'—'}</div></div><div class="fg"><div class="flabel">课包课时</div><div class="finput">${parseInt(p.packageLessons)||0} 节</div></div><div class="fg"><div class="flabel">课程类型</div><div class="finput">${esc(p.courseType)||'—'}</div></div><div class="fg"><div class="flabel">历史课程产品（辅助）</div><div class="finput">${esc(p.productName)||'—'}</div></div>${purchasePriceSummaryHtml(p)}<div class="fg"><div class="flabel">主归属教练</div><div class="finput">${esc(p.ownerCoach)||'—'}</div></div><div class="fg"><div class="flabel">可上课教练</div><div class="finput">${esc(parseArr(p.allowedCoaches).join('、'))||'—'}</div></div><div class="fg"><div class="flabel">支付方式</div><div class="finput">${esc(p.payMethod)||'—'}</div></div><div class="fg"><div class="flabel">购买状态</div><div class="finput">${purchaseStatusText(p)}</div></div><div class="fg"><div class="flabel">操作人</div><div class="finput">${esc(p.operator)||'—'}</div></div><div class="fg full"><div class="flabel">备注</div><div class="finput" style="min-height:42px">${esc(p.notes)||'—'}</div></div></div><div class="sec-ttl">课包余额</div><div class="fgrid"><div class="fg"><div class="flabel">当前余额</div><div class="finput">${ent?`${lessonQty(ent.remainingLessons)}/${lessonQty(ent.totalLessons)} 节`:'—'}</div></div><div class="fg"><div class="flabel">有效期</div><div class="finput">${ent?`${ent.validFrom||'—'} - ${ent.validUntil||'—'}`:'—'}</div></div><div class="fg"><div class="flabel">余额状态</div><div class="finput">${ent?entitlementStatusText(ent):'—'}</div></div></div><div class="sec-ttl">扣课记录</div>${purchaseLedgerHtml(p.id)}${purchasePackageSnapshotHtml(p)}${p.status==='voided'?`<div class="sec-ttl">作废信息</div><div class="fgrid"><div class="fg"><div class="flabel">作废时间</div><div class="finput">${esc(p.voidedAt)||'—'}</div></div><div class="fg"><div class="flabel">作废人</div><div class="finput">${esc(p.voidedBy)||'—'}</div></div><div class="fg full"><div class="flabel">作废原因</div><div class="finput" style="min-height:42px">${esc(p.voidReason)||'—'}</div></div></div>`:''}<div class="mactions"><button class="btn-cancel" onclick="closeModal()">关闭</button>${p.status==='voided'?'':`<button class="btn-save" onclick="openPurchaseEditModal('${p.id}')">编辑</button>`}</div>`;
...
function exportPurchaseCSV(){
  const list=getFilteredPurchases();
  let csv='购买日期,学员,售卖课包,课程类型,历史课程产品（辅助）,实收,剩余课时,总课时,有效开始,有效结束,支付方式,状态,备注\n';
  csv+=list.map(p=>{
    const ent=entitlements.find(e=>e.purchaseId===p.id)||{};
    return [p.purchaseDate||'',p.studentName||'',p.packageName||'',p.courseType||'',p.productName||'',parseFloat(p.amountPaid)||0,Number(ent.remainingLessons)||0,Number(ent.totalLessons)||0,ent.validFrom||'',ent.validUntil||'',p.payMethod||'',purchaseStatusText(p),'"'+String(p.notes||'').replace(/"/g,'""')+'"'].join(',');
  }).join('\n');
