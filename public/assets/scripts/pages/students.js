...
function openStudentPurchaseModal(studentId){
...
function studentFeedbackHistoryHtml(s){
  ...
  return rows.map(f=>{
    const sch=schedules.find(x=>x.id===f.scheduleId)||{};
    const course=[scheduleClassName(sch),sch.productName,scheduleCourseType(sch)].filter(Boolean).join(' / ')||'—';
...
function studentTeachingInfoHtml(stu){
  ...
  return `<div class="tms-section-header">教学信息</div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">当前状态</label><div class="finput tms-form-control"><span class="tms-tag ${status.badge==='b-green'?'tms-tag-green':status.badge==='b-red'?'tms-tag-red':'tms-tag-tier-blue'}">${status.label}</span></div></div><div class="tms-form-item"><label class="tms-form-label">课包 / 课时</label><input class="finput tms-form-control" value="${esc(studentPackageLessonSummary(stu))}" readonly></div></div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">最近上课</label><input class="finput tms-form-control" value="${recentSchedule?.startTime?daysAgoText(recentSchedule.startTime.slice(0,10)):'-'}" readonly></div><div class="tms-form-item"><label class="tms-form-label">累计上课</label><input class="finput tms-form-control" value="${studentCompletedLessonCount(stu)}" readonly></div></div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">负责教练</label><input class="finput tms-form-control" value="${esc(coachText)}" readonly></div><div class="tms-form-item"><label class="tms-form-label">当前班次</label><input class="finput tms-form-control" value="${esc(primaryClass?.className)||'未入班'}" readonly></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">班次进度（兼容查看）</label><div class="finput tms-form-control tms-readonly-text">${studentClassSummaryHtml(stu)}</div></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">上课记录</label><div class="finput tms-form-control tms-readonly-text">${studentLessonRecordHtml(stu)}</div></div></div><div class="tms-form-row" style="margin-bottom:0"><div class="tms-form-item full-width"><label class="tms-form-label">最近2条课后反馈</label><div class="finput tms-form-control tms-readonly-text">${recentFeedbacks.length?recentFeedbacks.map(f=>`${String(f.startTime||f.createdAt||'').slice(0,10)}：${f.practicedToday||f.knowledgePoint||f.nextTraining||'已填写反馈'}`).map(esc).join('<br>'):'-'}</div></div></div>`;
}
...
function studentConsumptionInfoHtml(stu){
  ...
  return `<div class="tms-section-header">消费与关联信息</div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">已购课包</label><div class="finput tms-form-control tms-readonly-text">${studentEntitlementSummaryHtml(stu)}</div></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">扣课记录</label><div class="finput tms-form-control tms-readonly-text">${studentEntitlementLedgerHtml(stu)}</div></div></div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">订场 / 会员</label><input class="finput tms-form-control" value="${esc(studentBookingMembershipSummary(stu))}" readonly></div><div class="tms-form-item"><label class="tms-form-label">最近订场</label><input class="finput tms-form-control" value="${latestCourtUseDateForStudent(stu)?daysAgoText(latestCourtUseDateForStudent(stu)):'—'}" readonly></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">课包购买记录</label><div class="finput tms-form-control tms-readonly-text">${studentEntitlementSummaryHtml(stu)}</div></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">课包消耗记录</label><div class="finput tms-form-control tms-readonly-text">${studentEntitlementLedgerHtml(stu)}</div></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">订场账户摘要</label><div class="finput tms-form-control tms-readonly-text">${studentAccountSummaryHtml(stu)}</div><div class="tms-field-help">关联订场账户在「订场/会员」页面编辑用户时选择「关联学员」。</div></div></div><div class="tms-form-row"><div class="tms-form-item full-width"><label class="tms-form-label">会员摘要</label><div class="finput tms-form-control tms-readonly-text">${studentMembershipSummaryHtml(stu)}</div></div></div>${linkedCourts.length?`<div class="tms-form-row" style="margin-bottom:0"><div class="tms-form-item full-width"><label class="tms-form-label">关联说明</label><div class="finput tms-form-control tms-readonly-text">${esc(linkedCourts.map(c=>c.name).join('、'))}</div></div></div>`:''}`;
}
...
async function saveStudent(){
  ...
  try{
    ...
    closeModal();toast(editId?'修改成功 ✓':'添加成功 ✓','success');renderStudents();renderSchedule();renderPurchases();renderEntitlements();renderMySchedule();
