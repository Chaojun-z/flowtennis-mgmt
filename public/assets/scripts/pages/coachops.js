  ...
  host.innerHTML=rows.map(r=>{
    if(mode==='day'){
      ...
      const blocks=r.rangeRows.sort((a,b)=>String(a.startTime).localeCompare(String(b.startTime))).map(s=>{
        ...
        return `<div class="coach-ops-block ${coachOpsCourseTypeTagClass(scheduleCourseType(s))}" style="left:${left}%;width:${Math.min(width,100-left)}%" onclick="event.stopPropagation();openScheduleDetail('${s.id}')"><div class="coach-ops-time">${s.startTime.slice(11,16)}${s.endTime?' - '+s.endTime.slice(11,16):''}</div><div class="coach-ops-student">${esc(s.studentName)||esc(scheduleClassName(s))||'—'}</div><div class="coach-ops-location">${esc(scheduleLocationText(s))}</div></div>`;
      }).join('');
