...
let courts=[],students=[],products=[],packages=[],purchases=[],entitlements=[],entitlementLedger=[],financialLedger=[],membershipPlans=[],membershipAccounts=[],membershipOrders=[],membershipBenefitLedger=[],membershipAccountEvents=[],pricePlans=[],schedules=[],coaches=[],classes=[],campuses=[],feedbacks=[],adminUsers=[],matches=[];
// 教学售卖治理口径：
...
function clearLoadedData(){
  ...
  schedules=[];coaches=[];classes=[];campuses=[];feedbacks=[];adminUsers=[];matches=[];adminUsersLoaded=false;
  loadedDatasets=new Set();
}
function normalizeCurrentPageForRole(){
  const isCoach=currentUser?.role==='editor'&&currentUser?.coachName;
  const retiredAdminPageRedirect={products:'packages',classes:'students',plans:'students'};
  if(isCoach){
  ...
  if(currentUser?.role==='admin'&&retiredAdminPageRedirect[currentPage]){
    currentPage=retiredAdminPageRedirect[currentPage];
    localStorage.setItem(PAGE_KEY,currentPage);
    return;
  }
...
function applyLoadedData(data){
  ...
  loadedDatasets=new Set(['courts','students','products','packages','purchases','entitlements','entitlementLedger','financialLedger','membershipPlans','membershipAccounts','membershipOrders','membershipBenefitLedger','membershipAccountEvents','pricePlans','schedule','coaches','classes','campuses','feedbacks','matches']);
  if(data?.user){
