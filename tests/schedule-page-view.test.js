...
assert.match(fnBody('refreshSchEntitlementOptions'), /sel\.innerHTML='[^']*重新[^']*课包[^']*';[\s\S]*apiCall\('POST','\/entitlements\/recommend'/, 'refreshing schedule entitlements should clear stale package options before async recommend returns');
...
assert.match(fnBody('onSchClassChange'), /cls\.courseType/, 'schedule class change should use the class snapshot course type');
assert.doesNotMatch(fnBody('onSchClassChange'), /products\.find\(p=>p\.id===cls\.productId\)/, 'schedule class change should not require product dataset lookups');
assert.match(source, /function findScheduleLinkedClass\(/, 'schedule page should centralize linked class lookup');
assert.match(fnBody('onSchClassChange'), /findScheduleLinkedClass\(/, 'schedule class change should reuse the linked class helper');
assert.match(fnBody('updateSchClassHint'), /findScheduleLinkedClass\(/, 'schedule class hint should reuse the linked class helper');
assert.match(fnBody('scheduleRemainingLessons'), /findScheduleLinkedClass\(/, 'schedule remaining lessons should reuse the linked class helper');
assert.doesNotMatch(fnBody('updateSchClassHint'), /classes\.find\(/, 'schedule class hint should not scatter direct class lookups');
assert.doesNotMatch(fnBody('scheduleRemainingLessons'), /classes\.find\(/, 'schedule remaining lessons should not scatter direct class lookups');
