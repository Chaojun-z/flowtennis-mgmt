...
async function main() {
  ...
  const getCachedScan = async (tableName, options) => {
    scanCalls.push({ tableName, options: options || null });
    return datasets[tableName] || [];
  };
  const getCachedRowsByIds = async (tableName, ids = []) => {
    rowCalls.push({ tableName, ids: [...ids] });
    const idSet = new Set((ids || []).map((item) => String(item || '').trim()).filter(Boolean));
    return (datasets[tableName] || []).filter((row) => idSet.has(String(row?.id || '').trim()));
  };
  ...
  const courtProjectionCall = scanCalls.find((call) => call.tableName === tables.courts);
  assert.ok(courtProjectionCall, '读模型应读取订场用户表');
  assert.deepStrictEqual(
    courtProjectionCall.options?.columns,
    [
      'name',
      'phone',
      'campus',
  ...
  assert.deepStrictEqual(rowCalls, [{ tableName: tables.courts, ids: ['court-1'] }], 'compare 固定样本应按账户 ID 取完整行');
  assert.strictEqual(
    scanCalls.filter((call) => call.tableName === tables.courts).length,
    1,
    'compare 应只保留一次首屏投影扫描，不应再做 legacy 全表 history 扫描'
  );
}
