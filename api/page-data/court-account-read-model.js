...
async function loadCourtAccountReadModelDeps({
  listCampusesWithDefaults,
  getCachedScan,
  tables,
  courts
} = {}) {
  const [campuses, students, loadedCourts, membershipAccounts, membershipOrders, membershipPlans] = await Promise.all([
    listCampusesWithDefaults(),
    getCachedScan(tables.students).catch(() => []),
    courts ? Promise.resolve(courts) : getCachedScan(tables.courts, { columns: COURT_ACCOUNT_LIST_PROJECTION_FIELDS }).catch(() => []),
...
function buildCourtAccountListViewResult({
  campuses = [],
  students = [],
  courts = [],
  membershipAccounts = [],
...
function createCourtAccountListViewLoader(deps) {
  ...
  return async function loadCourtAccountListView(options = {}) {
    ...
    const loaded = await loadCourtAccountReadModelDeps({
      listCampusesWithDefaults,
      getCachedScan,
      tables,
      courts: null
    });
    return buildCourtAccountListViewResult({
      ...loaded,
      useLegacy,
      sampleIds,
...
function createCourtAccountListCompareLoader(deps) {
  ...
  return async function loadCourtAccountListViewCompare(options = {}) {
    ...
    const view = buildCourtAccountListViewResult({
      ...loaded,
      useLegacy: false,
      sampleIds,
      sample: options.sample || ''
    ...
    const legacyCourts = sampleIds.length
      ? await getCachedRowsByIds(tables.courts, sampleIds).catch(() => [])
      : await getCachedScan(tables.courts).catch(() => []);
    const legacy = buildCourtAccountListViewResult({
      ...loaded,
      courts: legacyCourts,
      useLegacy: true,
      sampleIds,
